...
class Post {
	...
	public function submitPost($body, $user_to) {
		...
		if($check_empty != "") {
			...
			// insert notification
			if($user_to != 'none') {
				$notification = new Notification($this->con, $added_by);  // "profile_post" is type in function insertNotification
				$notification->insertNotification($returned_id, $user_to, "profile_post"); // return the id of last post
			}
	...
	public function loadProfilePosts($data, $limit) {
		...
		$str = ""; //String to return 
		$data_query = mysqli_query($this->con, "SELECT * FROM posts WHERE deleted='no' AND ((added_by='$profileUser' AND user_to='none') OR user_to='$profileUser') ORDER BY id DESC"); // if they posted and wasn't to anybody OR even they did not post it but it was to them
	...
	public function getSinglePost($post_id) {  // from function loadPostsFriends
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			$row = mysqli_fetch_array($data_query); 
				...
				$user_logged_obj = new User($this->con, $userLoggedIn);
				if($user_logged_obj->isFriend($added_by)){ // you can't see a post if you are not friends
			...
			echo "<p>Post not exist. If you clicked a link, it may be broken.</p>";
					return;
		}
