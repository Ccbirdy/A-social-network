...
$query = $_POST['query'];
$userLoggedIn = $_POST['userLoggedIn']; // element linked with " "

$names = explode(" ", $query);

if(strpos($query, "_") !== false) {    // look insde query trying to find "_" , integer and string , notice : here has 2 equals
	$usersReturned = mysqli_query($con, "SELECT * FROM users WHERE username LIKE '$query%' AND user_closed='no' LIMIT 8");
}   									// check if there is a "_",  check if literily like another
else if(count($names) == 2) {          // if in names array , search for first and last name
