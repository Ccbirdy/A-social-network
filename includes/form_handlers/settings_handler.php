...
if(isset($_POST['update_details'])) {
	...
	$email_check = mysqli_query($con, "SELECT * FROM users WHERE email='$email'"); // new email not used before
	$row = mysqli_fetch_array($email_check);
	$matched_user = $row['username'];

	if($matched_user == "" || $matched_user == $userLoggedIn) { // no one used that email or
